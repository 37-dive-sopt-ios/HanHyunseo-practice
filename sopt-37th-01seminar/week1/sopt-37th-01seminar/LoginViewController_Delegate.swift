...
class LoginViewController_Delegate: UIViewController {
    ...
    private func pushToWelcomeVC() {
        ...
        // 3️⃣ Delegate 설정 (핵심!)
        // welcomeVC의 delegate          LoginVC 자신 (self)
        // 메모리 관계:
        // LoginVC (self)
        //    ↓ strong 참조
        // WelcomeVC
        //    ↓ weak 참조
        // LoginVC (delegate로 참조)

        // 이제 WelcomeVC에서 delegate?.retryLogin() 호출하면
        // → LoginVC의 retryLogin 메서드가 실행됨!
    ...
    @objc
    // iOS개발은 원래 objectC로 하고있었음. 내부 프레임워크에서 아직 남아있기 때문에 그 코드를 스위프트 코드에서 사용할 수 있게 만들 수 있는 역할
